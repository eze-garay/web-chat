...
import { generateProduct } from "../utils.js";
      ...
      export async function getAllProducts(req, res) {
        ...
        try {
          ...
          res.render("products", { success: true,  ...result, docs: products, user: req.user });
        } catch (error) {
          console.log(error);
          res.render("products", { success: false, payload: { error: "NO SE PUDIERON OBTENER LOS PRODUCTOS" } });
        }
      }
      ...
      export async function getProduct(req, res) {
        try {
          ...
          if (one) {
            return res.status(200).send({ success: true, payload: one });
          }
        } catch (error) {
      ...
      export async function create(req, res) {
        ...
        try {
          ...
          if (prod) {
            return res.status(200).send({ success: true, payload: prod });
          }
        } catch (error) {
          console.log(error);
          return res.status(500).send({ success: false, payload: error.message });
      ...
      export async function deleteProduct(req, res) {
        ...
        try {
          ...
          return res.send({ success: true, payload: "Producto eliminado" });
        } catch (error) {
          return res.status(500).send({ success: false, payload: error.message });
        }
      }
      ...
      export async function updateProduct(req, res) {
        ...
        try {
          ...
          if (updatedProduct) {
            return res.status(200).send({ success: true, payload: { message: "Producto modificado" }})
          }
          
        } catch (error) {
          return res.status(500).send({ success: false, payload: { error: "El producto no se pudo modificar" } });
         }
        }
